...
public class DB_FindPW extends AsyncTask<String, String, String>
{
	boolean ok = false;
	Context exCon = null;
	...
	public DB_FindPW(String userName, String userEmail, String userID, Context exCon)
	{
		this.exCon = exCon;
	...
	protected String doInBackground(String... arg0)
	{
		...
		try
		{
			...
			String Query = "SELECT Password FROM User_Info WHERE Name = '"+userName+"' AND Email = '"+userEmail+"' AND ID = '"+userID+"';";
	...
	protected void onPostExecute(String result) // UI처리 구간
	{
		...
		try
		{
			...
			{
				// 로그인 성공
				((Find_Info_Activity) exCon).findPWSuccess(userPW);
			}
			else
			{
				((Find_Info_Activity) exCon).errorPWFind();
			}
		}
		catch (Exception e)
		{
